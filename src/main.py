...
def detector():
    ...
    global centers,black_chesses,white_chesses,help,mode_grid,dict
    # 检查摄像头是否成功打开
    # if not cap.isOpened():
    #     print("无法打开摄像头")
    #     exit()
    ...
    while True:
        ...
        try:
            ...
            black_chesses,white_chesses = find_chess_pro.findchess(frame)
...
# t1.join()
# t2.join()
camera_stream = CameraStream(camera_id = 0).start()
t1 = threading.Thread(target= detector)
t2 = threading.Thread(target= loop)
t3  = threading.Thread(target= grid_updater)
t1.start()
t2.start()
t3.start()